...
if __name__ == '__main__':
    ...
    for model_name in args.model_name:
        ...
        design_only_positions = ' '.join([str(i) for i in range(1, args.peptide_length + 1)])
        ...
        os.system(f'python ../helper_scripts/make_fixed_positions_dict.py --input_path={path_for_parsed_chains} --output_path={path_for_fixed_positions} --chain_list "{args.peptide_chain}" --position_list "{design_only_positions}" --specify_non_fixed')
        ...
        print(model_name)
        pdbids = [f.split('.')[0] for f in os.listdir(args.folder_with_pdbs) if f.endswith('.pdb')]
        sequences_per_pdbid = {}
        for pdbid in pdbids:
            print(pdbid)
            sequences = []
        ...
        header = 'organism	mhc_class	mhc	peptide	va	ja	cdr3a	vb	jb	cdr3b	pdbid	hcnn_model	-log10(Kd)	hamming_distance_from_WT	hcnn_pnE	pnE pdbid'
        template_row = 'human	1	A*02:01	{seq}	TRAV21*01	TRAJ6*01	CAVRPTSGGSYIPTF	TRBV6-5*01	TRBJ2-2*01	CASSYVGNTGELFF	from_kd_data	from_kd_data	-0.69284691927723	1	inf    {pdbid}'
